...
User* DBParser::parseUser(istream& is)
{
  ...
  if( is.fail() ){
    error_ = true;
    errorMsg_ = "Unabel to read password";
  }
  if(error_){
    return NULL;
  }
...
Review* DBParser::parseReview(istream& is, string prodname)
{
  ...
  if(error_){
    ...
    return makeReview(prodname, rating, date, review_text, username);
  }
...
bool DBParser::parseDictionary(std::string db_filename, DataStore& ds)
{
  string first;
  string second;
  string line;
  ...
  while(getline(ifile, line)){
    ...
    if(dict.find(first)==dict.end()){
      ...
      set<string> temp;
      temp.insert(second);
      dict.insert(make_pair(first,temp));
    }
    else{
...
void DBParser::parseLine( std::istream& is){
  string first;
  string second;
